...
- (About page) Change Share button location to side of the app logo - Removed Contributions Page
- (Full Timetable page) Fix appbar appearane to increase visibility of background image & text.
- Upgrade dependencies


<ChangelogHeader version="2.12.5" date="1 September 2024" githubSlug="2.12.5+150" />

- Fixes issue where **homescreen widget** failed to update in September due to incorrect locale setting. See [#244](https://github.com/mptwaktusolat/app_waktu_solat_malaysia/issues/244)
- Add **onboarding coachmarks** for first installs (or first open after the update)
- Upgrade dependencies
...
- Temporary fix the offsetted **hijri date** [#54](https://github.com/mptwaktusolat/app_waktu_solat_malaysia/issues/54), I just change the assignation for `GetStorage()` when app started. This will affect only for new installs, where `GetStorage()` object not existed yet, existing user will need to adjust the hijri date manually. Guide to change offset date can be find [here](http://bit.ly/mpthijrioffset).
