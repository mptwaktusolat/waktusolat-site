...
- Fixed the long lasting **wrong hijri** date issue [#54](../issues/54). Hijri date is now consume directly from JAKIM.
...
- Initial implementation of **Semantics**. [#97](../issues/97)
- Other changes
...
- Upgrade dependencies including few major. Commit ref [`215d46a`](../commit/215d46ab34ce63bb5b4f4fb4f2460ccfc09e6949)
...
- Fixed **empty text** in Zone Chooser - When `locality` is empty, use `name` instead. [#67](../issues/67)
- Removed `kStoredLocationLocality` key (no longer needed).
- Re-enable **Salam Jumaat** wish in notification. [#81](../issues/81)
...
- Fix Trying to read MMM from 01 Okt-2021 05:43:00 at position 3`. Date format from JAKIM is **not standard** so the app can't parse it. Issue [#86](../issues/86).
- Update changelog link in About Page.

<ChangelogHeader version="2.2.3" date="28 September 2021" githubSlug="2.2.3+79" />

- :sweatFixed time from API is being **wrongly parsed** as 12-Hour system (supposedly 24-Hour system), causing wrong time shown for some zones *Zohor time in Sabah)\_. Issue [#85](../issues/85). Thank you, anonymous user for alerting me about this issue. (Yes, this is repeated issue similar to [this](#ver-11389-hotfix---19-february-2021) update back then :man_facepalming:)
- Minor UI change manual location chooser - cleaner header textstyle.
- Temporary disable `Salam Jumaat` wish (`summary`) in notification. Issue [#81](../issues/81).

<ChangelogHeader version="2.2.2" date="22 September 2021" githubSlug="2.2.2+78" />

- Some improvement in **prayer time timetable**:
  - Fixed a **typ~~Imsak~~ Syuruk. [#80](../issues/80).
...
- **API Migration** from mpti906 to JAKIM e-solat API. Fixed [#83](../issues/81). A part of [#82](../pull/82). [Why?](../pull/82#issuecomment-927835374)
...
- Added **azan notification**. Now there are two types of notification, default sound and azan. Existing user will be asked if they want to try. Source: https://soundcloud.com/alafasy/sets/athan. Closed [#39](../issues/39):
...
- [firebase_remote_config](https://pub.dev/packages/firebase_remote_config). Hijri offset data now fetched from the server (controlled by me so remind me if I forgot to update, allow up to 8 hours for the changes fully propagates), removed manual adjustment setting. Issue [#54](../issues/54).
...
- New **Monthly Prayer Table**, featuring beautiful [IIUM SHAS Mosque](https://i2.wp.com/news.iium.edu.my/wp-content/uploads/2017/06/10982272836_29abebc100_b.jpg?ssl=1) :mosque::heart:. [#55](../issues/55)
...
- Throw error when detected location is **not in Malaysia**. Commit ref [b500e4c](../commit/b500e4c9ef15db4c69f2fef6a0e25f4a0ede5e49). Also fixed `invalid argument(s)` error.
...
- Fix attempt for the [#56](../issues/56). Not sure about the issue tbh.
- Fixed **offset Hijri date** on both web and Android (Issue [#59](../issues/59)) - Ability to adjust the offset by +-2 days.
